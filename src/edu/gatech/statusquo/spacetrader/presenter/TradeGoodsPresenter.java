...
public class TradeGoodsPresenter {
    ...
    private void setListeners() {
        /**
         * Mouse listener for the marketplace, allows entering of quantities for
         * buying
         */
        ...
        tradeGoodsView.btnBuy.addMouseListener(new MouseAdapter() {
            public void mouseUp(MouseEvent e) {
                ...
                /**
                 * Throws error message if no amount is entered when the player
                 * attempts a transaction
                 */

                if (quantText.equals("") || quantText.equals("Enter Qty")) {
                    NotificationsView.list_1
                            .add("Please enter an amount in the text box");
                    return;
                }
                ...
                try {
                    ...
                    if (quant < 0) {
                        quant = 0;
                        NotificationsView.list_1
                                .add("Please enter a POSITIVE integer!");
                    }
                ...
                /**
                 * Check to make sure a player has selected a good before
                 * attempting a transaction
                 */

                if (tradeGoodsView.table_1.getSelection().length == 0) {
                    NotificationsView.list_1
                            .add("Please select an item from the market");
                    return;
                } else {
                    // int price =
                    // Integer.parseInt(tradeGoodsView.table_1.getSelection()[0].getText(1));
                    GoodType goodType = GoodType.valueOf(tradeGoodsView.table_1
                    ...
                    /**
                     * Check to make sure the player has enough currency to
                    ...
                    if (Player.getCurrency() < quant * price) {
                        ...
                        NotificationsView.list_1
                                .select(NotificationsView.list_1.getItemCount() - 1);
                        NotificationsView.list_1.showSelection();
                        return;
                        /**
                         * Check to make sure enough of quantity is available to
                         * purchase
                         */
                        ...
                        /**
                         * Check to make sure the player has enough room in the
                         * cargobay for the new goods
                         */
                    } else if (quant > cargoCountLeft) {
                        NotificationsView.list_1
                                .add("Sorry, you do not have enough space in your cargo bay.");
                        NotificationsView.list_1
                                .select(NotificationsView.list_1.getItemCount() - 1);
                        NotificationsView.list_1.showSelection();
                        return;
                        ...
                        player.setCurrency(Player.getCurrency()
                                - (quant * price));
                        // going to decrement the markets quantity depending qty
                        // bought
                        // getCurrentQty - qtyBought = newMarketQty,
                        // puts(goodtype, newmarketqty)
                        int newMarketQty = (marketQuantity.get(goodType))
                                - quant;
        ...
        tradeGoodsView.btnSell.addMouseListener(new MouseAdapter() {
            public void mouseUp(MouseEvent e) {
                ...
                /**
                 * Check to make sure an item is selected before attempting
                 * transaction
                 */
                if (tradeGoodsView.table_1.getSelection().length == 0) {
                    NotificationsView.list_1
                    ...
                    if (quant > cargoCount) {
                        ...
                        return;
                        /**
                         * If checks pass the items are removed from cargobay
                         * and the market pays the player
                         */
                    } else {
                        ship.removeCargo(quant, goodType);
                        player.setCurrency(Player.getCurrency()
                                + (quant * price));
                        // going to increment the markets quantity depending qty
                        // sold
                        // getCurrentQty + qtyBought = newMarketQty,
                        // puts(goodtype, newmarketqty)
                        int newMarketQty = (marketQuantity.get(goodType))
        ...
        tradeGoodsView.btnBuyFuel.addMouseListener(new MouseAdapter() {
            public void mouseUp(MouseEvent e) {
                ...
                if (quantText.equals("") || quantText.equals("Enter Qty")) {
                    NotificationsView.list_1
                            .add("Please enter an amount in the text box");
                    NotificationsView.list_1.select(NotificationsView.list_1
                            .getItemCount() - 1);
                    NotificationsView.list_1.showSelection();
                    return;
                }
                ...
                if (fuelCapacity < fuelLevel + quant) {
                    NotificationsView.list_1
                            .add("This amount of fuel would cause your tank to burst");
                    NotificationsView.list_1.select(NotificationsView.list_1
                            .getItemCount() - 1);
                    NotificationsView.list_1.showSelection();
                    return;
                    ...
                    NotificationsView.list_1
                            .add("Purchasing this amount of fuel would put you in the poor house");
                    NotificationsView.list_1.select(NotificationsView.list_1
                            .getItemCount() - 1);
                    NotificationsView.list_1.showSelection();
                    return;
    ...
    public static void fillTradeGoodsTable() {
        ...
        String[] water = { "WATER",
                Integer.toString(marketPrice.get(GoodType.WATER)),
                Integer.toString(marketQuantity.get(GoodType.WATER)), "N/A" };
        TradeGoodsView.waterItem.setText(water);
        String[] fur = { "FUR",
                Integer.toString(marketPrice.get(GoodType.FUR)),
                Integer.toString(marketQuantity.get(GoodType.FUR)), "N/A" };
        TradeGoodsView.furItem.setText(fur);
        String[] food = { "FOOD",
                Integer.toString(marketPrice.get(GoodType.FOOD)),
                Integer.toString(marketQuantity.get(GoodType.FOOD)), "N/A" };
        TradeGoodsView.foodItem.setText(food);
        String[] ore = { "ORE",
                Integer.toString(marketPrice.get(GoodType.ORE)),
                Integer.toString(marketQuantity.get(GoodType.ORE)), "N/A" };
        TradeGoodsView.oreItem.setText(ore);
        String[] firearms = { "FIREARM",
                Integer.toString(marketPrice.get(GoodType.FIREARM)),
                Integer.toString(marketQuantity.get(GoodType.FIREARM)), "N/A" };
        TradeGoodsView.firearmsItem.setText(firearms);
        String[] medicine = { "MEDICINE",
                Integer.toString(marketPrice.get(GoodType.MEDICINE)),
                Integer.toString(marketQuantity.get(GoodType.MEDICINE)), "N/A" };
        TradeGoodsView.medicineItem.setText(medicine);
        String[] machines = { "MACHINE",
                Integer.toString(marketPrice.get(GoodType.MACHINE)),
                Integer.toString(marketQuantity.get(GoodType.MACHINE)), "N/A" };
        TradeGoodsView.machinesItem.setText(machines);
        String[] narcotics = { "NARCOTIC",
                Integer.toString(marketPrice.get(GoodType.NARCOTIC)),
                Integer.toString(marketQuantity.get(GoodType.NARCOTIC)), "N/A" };
        TradeGoodsView.narcoticsItem.setText(narcotics);
        String[] robots = { "ROBOT",
                Integer.toString(marketPrice.get(GoodType.ROBOT)),
                Integer.toString(marketQuantity.get(GoodType.ROBOT)), "N/A" };
